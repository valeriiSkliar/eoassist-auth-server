...
export const AgreementCheckbox: React.FC = () => {
  const { isAgreed, setIsAgreed, highlightCheckbox, isHighlighted } = useDataAgreement();
  
  const t = useTranslations('signIn');


return (
    <div className={`p-4 border-l-4 ${isHighlighted ? 'bg-yellow-50 border-yellow-400' : ''} ${isAgreed ? 'bg-green-50 border-green-300' : ''} transition-all duration-300`}>
      <div className="flex items-center space-x-3">
        <div className={`flex-shrink-0 ${isHighlighted ? 'text-yellow-400' : 'text-gray-400'}`}>
        ...
        <div className="flex-1">
          ...
          <label htmlFor="terms" className="ml-2 text-sm text-gray-700">
            ...
            <Link target='_blank' href="https://eoassist.com/privacy-policy" className="text-yellow-600 hover:underline ml-1">
              Privacy Policy
            </Link>
          </label>
        </div>
